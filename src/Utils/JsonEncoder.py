...
def specific_index_creation():
    ...
    for entity in entity_array:
        result_specific_index = db.cypher_query(
            "CALL db.index.fulltext.createNodeIndex('" + entity + "',['" + entity + "'],['name','uid'])")
...
def search_specific_cidoc(entity, name):
    if name.startswith('"') and name.endswith('"'):
        results, meta = db.cypher_query("CALL db.index.fulltext.queryNodes('" + entity + "','" + name + "')")
    else:
        results, meta = db.cypher_query("CALL db.index.fulltext.queryNodes('" + entity + "','" + name + "~')")
    results_list = []
