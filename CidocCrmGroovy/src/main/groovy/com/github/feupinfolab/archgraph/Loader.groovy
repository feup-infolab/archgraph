package com.github.feupinfolab.archgraph

import com.github.feupinfolab.archgraph.cidoc.nodeTraits.E1_CRM_Entity_T
import com.github.feupinfolab.archgraph.cidoc.nodes.AE1_Level_of_Description
import com.github.feupinfolab.archgraph.cidoc.nodes.E10_Transfer_of_Custody
import com.github.feupinfolab.archgraph.cidoc.nodes.E12_Production
import com.github.feupinfolab.archgraph.cidoc.nodes.E1_CRM_Entity
import com.github.feupinfolab.archgraph.cidoc.nodes.E21_Person
import com.github.feupinfolab.archgraph.cidoc.nodes.E22_E33
import com.github.feupinfolab.archgraph.cidoc.nodes.E22_E38
import com.github.feupinfolab.archgraph.cidoc.nodes.E22_ManMade_Object
import com.github.feupinfolab.archgraph.cidoc.nodes.E28_Conceptual_Object
import com.github.feupinfolab.archgraph.cidoc.nodes.E33_Linguistic_Object
import com.github.feupinfolab.archgraph.cidoc.nodes.E35_Title
import com.github.feupinfolab.archgraph.cidoc.nodes.E40_Legal_Body
import com.github.feupinfolab.archgraph.cidoc.nodes.E41_Appellation
import com.github.feupinfolab.archgraph.cidoc.nodes.E42_Identifier
import com.github.feupinfolab.archgraph.cidoc.nodes.E52_Time_Span
import com.github.feupinfolab.archgraph.cidoc.nodes.E54_Dimension
import com.github.feupinfolab.archgraph.cidoc.nodes.E55_Type
import com.github.feupinfolab.archgraph.cidoc.nodes.E56_Language
import com.github.feupinfolab.archgraph.cidoc.nodes.E74_Group
import com.github.feupinfolab.archgraph.cidoc.nodes.E7_Activity
import com.github.feupinfolab.archgraph.cidoc.nodes.E84_E33
import com.github.feupinfolab.archgraph.cidoc.nodes.PC14_Carried_Out_By
...
import com.github.feupinfolab.archgraph.db.DBConnection
...
class Loader {
    ...
    void process () {
        ...
        //loadApocalipse()
        loadApArchive()
        //Iterable<E1_CRM_Entity_T> t = query6(session,"E12_Production")
        //String json = jsonQuery()
        //println(json)
        //json = json.substring(1,json.size())
        //String jsonLd = jsonToJsonLd(json)
        //loadJsonLd(jsonLd)
    }

    @CompileDynamic
    private void addNode(String json){

        JsonSlurper slurper = new JsonSlurper()
        Map result = (Map) slurper.parseText(json)

        E1_CRM_Entity_T instance = (E1_CRM_Entity_T) Class.forName("cidoc.nodeEntities." + (String) result."@type").getDeclaredConstructor().newInstance()
        ...
        for(int i = 4; i< result.size(); i++){
            ...
            for(int j = 0; j<relationsList.size();j++){
                ...
                E1_CRM_Entity_T targetNode = (E1_CRM_Entity_T) session.load(E1_CRM_Entity_T,id)
...
//    private void deleteNode(String json){
//
//        JsonSlurper slurper = new JsonSlurper()
//        Map result = (Map) slurper.parseText(json)
//        String id = result."@id"
...
//        }
//
//        E1_CRM_Entity_T ent = session.load(E1_CRM_Entity_T,id)
//        session.delete(ent)
//
//        txn.commit()
//
...
//    }
    ...
    @CompileDynamic
     private void loadJson(String json){
         ...
         Map<Long, E1_CRM_Entity_T> entities = new HashMap<>()

         //For cycle to create all Nodes
         for(int i = 0; i < nodes.size(); i++){
             Map entityMap =  (Map) nodes.get(i)
             ArrayList entityArray = (ArrayList) entityMap.label
             String E1_CRM_Entity_T = (String) entityArray.get(1)
             //Neo4j Java creation isnt always consistent, this makes sure the right E1_CRM_Entity_T gets created
             if(E1_CRM_Entity_T == "Entity"){
                 E1_CRM_Entity_T = (String) entityArray.get(0)
             }

             E1_CRM_Entity_T instance = (E1_CRM_Entity_T) Class.forName("cidoc.nodeEntities." + entity).getDeclaredConstructor().newInstance()
        ...
        //For cycle to create relationships
         for(int i = 0; i< relationships.size(); i++){
            ...
            E1_CRM_Entity_T e1 = entities.get((Long) fromNode.id)
            E1_CRM_Entity_T e2 = entities.get((Long) toNode.id)
    ...
    void  loadJsonLd(String jsonld){
       ...
       // String json = jsonld.substring(1,jsonld.size())
        ...
        Map<Long, E1_CRM_Entity> entities = new HashMap<>()

        for(int i =0; i< nodes.size(); i++){
            Map entityMap =  (Map) nodes.get(i)
            String E1_CRM_Entity_T =  entityMap."@type"

            if(E1_CRM_Entity_T == "Entity"){
                E1_CRM_Entity_T = "E52_Time_Span"
            }
            E1_CRM_Entity_T instance

            if(!entities.containsKey((Long) entityMap."@id")){


            instance = (E1_CRM_Entity_T) Class.forName("cidoc.nodeEntities." + entity).getDeclaredConstructor().newInstance()
            ...
            for(int j = 4; j< keys.size(); j++){
                ...
                E1_CRM_Entity_T instance2

                if(!entities.containsKey((Long) relations."@id")){

                    instance2 = (E1_CRM_Entity_T) Class.forName("cidoc.nodeEntities." + entity2).getDeclaredConstructor().newInstance()
    ...
    String getAllRelations(String entityType){
        ...
        E1_CRM_Entity_T instance
        instance = (E1_CRM_Entity_T) Class.forName("cidoc.nodeEntities." + entityType).getDeclaredConstructor().newInstance()
    ...
    Map getCounterDomain(String entityType, String relation, Map targets){
        ...
        for(int i =0; i< targets.size()-2; i++){
            ...
            E1_CRM_Entity_T instance2 = (E1_CRM_Entity_T) Class.forName("cidoc.nodeEntities." + targetType).getDeclaredConstructor().newInstance()
    ...
    static private void loadApocalipse () {
        db.withTransaction {
            def e22E33 = new E22_E33()
            def e22E33_2 = new E22_E33()

            def e84E33 = new E84_E33()

            def e22E38 = new E22_E38()

            def e55 = new E55_Type()
            def e55_2 = new E55_Type()
            def e55_3 = new E55_Type()
            def e55_4 = new E55_Type()


            def e35 = new E35_Title()
            def e35_2 = new E35_Title()
            def e35_3 = new E35_Title()
            def e35_4 = new E35_Title()

            def e33 = new E33_Linguistic_Object()

            def e28 = new E28_Conceptual_Object()

            def e7  = new E7_Activity()
    ...
    static private void loadApArchive(){
        db.withTransaction {
            ...
            xsd_integer.setValue(1.toString())
            xsd_string.setValue("Test")
            xsd_date.setValue(date.toString())
    ...
    static private void loadCalendario() {
        db.withTransaction {
            ...
            xsd_5.setValue("1 doc. impresso (315 x 222 mm.)")
            xsd_6.setValue("Relação sucessora: Portugal, Torre do Tombo, Reforma das Gavetas, liv. 6, f. 63 v.")
            xsd_7.setValue("papel")
            xsd_8.setValue("24/3/2015")
            xsd_9.setValue("31/1/2008")
...
//    }
//
//    static private Iterable<E1_CRM_Entity_T> queryForTernary(Session session){
//
...
//    }
//
//    static private Iterable<E1_CRM_Entity_T> queryTest(Session session){
//
...
//    }
//
//    static private Iterable<E1_CRM_Entity_T> queryAll(Session session){
//
...
//    }
//
//    static private Iterable<E1_CRM_Entity_T> query1(Session session){
//
...
//
//    }
//    static private Iterable<E1_CRM_Entity_T> query2(Session session){
//
...
//    }
//
//    static private Iterable<E1_CRM_Entity_T> query3(Session session){
//
...
//    }
//
//    static private Iterable<E1_CRM_Entity_T> query4(Session session){
//
...
//    }
//
//    static private Iterable<E1_CRM_Entity_T> query5(Session session){
//
...
//    }
//
//    static private String jsonQuery(){
//        Collection<E1_CRM_Entity_T> entities = session.loadAll(E1_CRM_Entity_T)
//
